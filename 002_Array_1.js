// array도 const로 선언한다
// const 는 값의 변화가 안된다 ? ->
// arr 가 가르키는 메모리 주소는 a1이다 따라서 가르키는 곳이 같기 때문에
// a1안에 값이 변해도 상관없다
// 하지만 arr = 100; 은 안된다
// 변수는 대부분 const로 짓는다
...
// 두개가 같은 얘기입니다.
arr.length = 10; // arr의 길이를 10이라고 정해준것 배열3개와 7개의 값이 늘어나게 된다
arr[0] = 100;
...
const arr = Array(100).fill(0);
arr.map((v, i) => {
  return i * 2; //(100) [0, 2, 4, 6, 8, 10, ... 198]
});
...
const ages1 = tip12.map((item) => item.age); // tip12에서 []안의 { }의 한덩어리인 item을 순회를 돌아서 age만 뽑아서 ages 변수에 전달
